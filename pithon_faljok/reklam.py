...
def osszes(varos,nap):
    return sum([sor.db for sor in lista if sor.varos == varos and sor.nap ==nap])
...
#pl = (sum([sor.db for sor in lista if sor.nap == 21 and sor.varos == "PL"]))
#tv = (sum([sor.db for sor in lista if sor.nap == 21 and sor.varos == "TV"]))
#nr = (sum([sor.db for sor in lista if sor.nap == 21 and sor.varos == "NR"]))
...
"""
8. Összesítse városonként, hogy hány rendelés érkezett az első 10, a 11-20-adik valamint a záró
10 napon! Az eredményt (a fejlécet is beleértve) táblázatos formában, tabulátorokkal
tagoltan jelenítse meg a képernyőn, illetve írja azonos formátumban a kampany.txt
szöveges állományba! 
"""
